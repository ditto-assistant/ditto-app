...
export async function syncUserData(userID: string, onProgress?: (stage: number, status: string) => void): Promise<Result<void>> {
            ...
            for (const line of lines) {
              try {
                ...
                if (onProgress && data.status) {
                  ...
                  if (data.status.includes("Step 1/3") || data.status.includes("Syncing memories from Firestore")) {
                    stage = 1
                  } else if (data.status.includes("Step 2/3") || data.status.includes("Generating subjects")) {
                    stage = 2
                  } else if (data.status.includes("Step 3/3") || data.status.includes("Storing generated data") || data.status.includes("Storing KG data")) {
                    stage = 3
                  } else if (data.status.includes("complete") || data.status.includes("Complete") || data.status.includes("storage complete") || data.status.includes("sync completed") || data.status === "complete") {
