...
const LLMTest: React.FC = () => {
  const [userPrompt, setUserPrompt] = useState("Tell me about a fun adventure in 5 sentences.");
  ...
  const testV1 = async () => {
    ...
    try {
      ...
      setV1Response("Error: " + (error instanceof Error ? error.message : String(error)));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const testV2 = async () => {
    ...
    try {
      ...
      setV2Response("Error: " + (error instanceof Error ? error.message : String(error)));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <label style={{ display: 'block', marginBottom: '5px' }}>User Prompt:</label>
        <textarea 
          value={userPrompt} 
