...
export default function SendMessage({
  ...
  const { isMobile, isIOS, isPWA } = usePlatform(); // Get additional platform info
  const {
    refetch,
  ...
  // Add useEffect to update CSS variable with the footer height
  useEffect(() => {
        ...
        if (followButton) {
          ...
          if (isIOS) {
            ...
            if (isPWA) {
              ...
              // Safari needs even more space for the browser chrome
              bottomPosition += safeAreaValue + 40;
            }
          } else if (isMobile) {
