...
const LiveModeModal: React.FC<LiveModeModalProps> = ({
  ...
  const handleSendMessage = async (message: string) => {
    if (!user?.uid || !userData || isProcessing || !preferences.preferences) return
  ...
  const hasAnyVoiceSupport = isSpeechRecognitionSupported || isSpeechSynthesisSupported
      ...
      {isOpen && (
            ...
            {isSpeechSynthesisSupported && (
              <Button
                ...
                className={cn(
                  ...
                  isAutoSpeak && "bg-green-100 dark:bg-green-900/20 text-green-600"
                )}
                title={isAutoSpeak ? "Disable voice readback" : "Enable voice readback"}
              >
                {isAutoSpeak ? <Volume2 className="h-4 w-4" /> : <VolumeX className="h-4 w-4" />}
              </Button>
            )}
        ...
        {!hasAnyVoiceSupport && (
          <div className="bg-amber-50 dark:bg-amber-950/20 text-amber-800 dark:text-amber-200 p-3 rounded-md text-sm">
            Voice features are not supported in your browser. Please try Chrome for the best experience.
          </div>
        )}
            ...
            <div className="flex justify-start">
              <div className="bg-muted rounded-lg p-3 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-muted-foreground">Ditto is thinking...</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="flex justify-start">
              <div className="bg-green-100 dark:bg-green-900/20 rounded-lg p-3 text-sm border border-green-200 dark:border-green-800">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-700 dark:text-green-300">Ditto is speaking...</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="text-center mt-2 text-xs text-muted-foreground">
            ...
            {isSpeechSynthesisSupported && (
              <div className={cn(
                ...
                {isAutoSpeak ? "🔊 Voice responses ON" : "🔇 Voice responses OFF"}
              </div>
            )}
          </div>
        </div>
