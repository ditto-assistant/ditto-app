...
const isModelAccessible = (model: LLMModel | ImageModel, userTier: number = 0) => {
  // Special case for LLaMA 4 models - always accessible
  if ('costPerMillionInputTokens' in model && model.name.toLowerCase().includes("llama-4")) {
    return true;
  }
  ...
  if (model.costPerMillionInputTokens > 30 && userTier < 3)
    return false;
  if (model.costPerMillionInputTokens > 10 && userTier < 2)
    return false;
  if (model.costPerMillionInputTokens > 0 && userTier < 1)
    return false;
...
export const ModelCard = ({
  ...
  const modelCost = isLLMModel ? model.costPerMillionInputTokens || 0 : (model as ImageModel).cost;
  const tierLabel = isLlama4 ? "Free" : (modelCost > 0 ? "Premium" : "Free");
  
  // Function to get upgrade message
  const getUpgradeMessage = (cost: number) => {
    if (userTier === 0) return { text: "Sign in to access", icon: <FaCrown /> };
  ...
  return (
    ...
    >
      <CardHeader className="p-4 pb-0 flex flex-row items-center justify-between space-y-0">
        <div>
          <CardTitle className="text-base font-semibold">{model.displayName}</CardTitle>
      ...
      <CardContent className="p-4">
        <div className="flex flex-wrap gap-2 mb-2">
          ...
          <Badge
            ...
            className="flex items-center gap-1"
          >
            {modelCost > 0 && !isLlama4 ? <FaCrown /> : <FaCrown style={{ opacity: 0.5 }} />}
            <span>{tierLabel}</span>
          </Badge>
      ...
      <CardFooter className="p-4 pt-0 flex justify-between items-center">
        ...
        {!isAccessible && (
          <Badge variant="outline" className="cursor-pointer" onClick={(e) => {
            ...
            <span className="ml-1">{getUpgradeMessage(modelCost).text}</span>
          </Badge>
        )}
      </CardFooter>
        ...
        <div className="border-t p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.values(IMAGE_GENERATION_SIZES).map((size) => (
              <Button
                key={size.wh}
                variant={preferences?.imageGeneration?.size?.wh === size.wh ? "default" : "outline"}
                size="sm"
                onClick={(e) => {
                  e.stopPropagation();
