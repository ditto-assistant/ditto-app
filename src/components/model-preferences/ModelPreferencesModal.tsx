...
export const ModelPreferencesModal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"main" | "programmer" | "image">("main");
  ...
  const filteredPromptModels = useMemo(() => {
    ...
    if (activeFilters.modelFamily) {
      filtered = filtered.filter(
        (model) => (model.modelFamily || model.provider) === activeFilters.modelFamily,
      );
    }
  ...
  const filteredImageModels = useMemo(() => {
    ...
    if (imageFilters.dimensions) {
      filtered = filtered.filter(
        (model) => model.imageOrientation.toLowerCase() === imageFilters.dimensions,
      );
    }
    ...
    if (imageFilters.modelFamily) {
      filtered = filtered.filter(
        (model) => (model.modelFamily || model.provider) === imageFilters.modelFamily,
      );
    }
  ...
  return (
    <div className="bg-background text-foreground w-full h-full flex flex-col overflow-hidden">
      <Tabs 
        defaultValue="main" 
        value={activeTab} 
        onValueChange={(val) => setActiveTab(val as "main" | "programmer" | "image")} 
      ...
      >
        ...
        <TabsContent value="main" className="h-[calc(100vh-8rem)] flex flex-col md:flex-row overflow-hidden">
          <ModelFilters
        ...
        <TabsContent value="programmer" className="h-[calc(100vh-8rem)] flex flex-col md:flex-row overflow-hidden">
          <ModelFilters
            activeFilters={activeFilters}
            setActiveFilters={setActiveFilters}
            filterType="prompt"
        ...
        <TabsContent value="image" className="h-[calc(100vh-8rem)] flex flex-col md:flex-row overflow-hidden">
          <ModelFilters
