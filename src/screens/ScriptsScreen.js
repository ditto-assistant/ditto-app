...
const ScriptsScreen = () => {
  ...
  const handleLaunchScript = () => {
    ...
    const selected =
      scripts.webApps.find((script) => script.name === selectedScript) ||
      scripts.openSCAD.find((script) => script.name === selectedScript);
    if (selected) {
        ...
        if (newestScript) {
          ...
          if (selectedScript === currentScript.name) {
            ...
            setCurrentVersion((prev) => ({
              ...prev,
              [category]: { ...newestScript, name: newName },
            }));
          }
  ...
  const handleOverlayClick = (event) => {
    if (
      versionOverlayRef.current &&
      !versionOverlayRef.current.contains(event.target)
    ) {
      setVersionOverlay(null);
    }
  };
  ...
  const getBaseName = (name) => {
    ...
    let res = match
      ? match[0].replace(/([a-z])([A-Z])/g, "$1 $2").trim()
      : name;
    return res;
  };
  ...
  const renderVersionOverlay = (baseName, scriptsList, cardRect) => {
    ...
    const currentlyDisplayedScript =
    ...
    const filteredScriptsList = scriptsList.filter((script) => {
      ...
      const isCurrentlyDisplayedLatest =
        isLatest && script.name === currentlyDisplayedScript?.name;
      ...
      if (isCurrentlyDisplayedLatest && !selectedScript?.startsWith(baseName))
        return false;
              ...
              <div style={styles.scriptCardHeader}>
                {renameScriptId === currentScript.id ? (
                  ...
                  <p
                    style={{
                      ...styles.scriptName,
                      fontSize: getFontSize(currentScript.name),
                    }}
                  >
                    {scriptBaseName}
                    {version && (
  ...
  useEffect(() => {
    const loadScripts = async () => {
      ...
      if (userID) {
        ...
        const localWebApps = JSON.parse(localStorage.getItem("webApps")) || [];
        const localOpenSCAD =
          JSON.parse(localStorage.getItem("openSCAD")) || [];
        setScripts({
          webApps: localWebApps,
          openSCAD: localOpenSCAD,
        });
